...
class DelayedRequeue {
  ...
  async create ({ queueName, ttl, expire, routingKey = undefined, exchangeName = undefined, logger: customLogger }) {
  ...
  async sendQueueMessage (message) {
    const res = this.ch.sendToQueue(`retry_${this.name}`, Buffer.from(JSON.stringify(message)))
    if (res) return true
    else {
