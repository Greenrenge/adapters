...
class Queue extends EventEmitter {
  ...
  async connect () {
    ...
    this.consumerTag = `${hostname}.${timestamp}.${Math.round(Math.random() * 100)}`
  }

  async consume () {
    this.ch.consume(this.queueName, (msg) => {
      try {
  ...
  async sendQueueMessage (message) {
    const res = this.ch.sendToQueue(this.queueName, Buffer.from(JSON.stringify(message)))
    if (res) return true
    else {
  ...
  async sendExchangeMessage ({ exchangeName, message, routingKey }) {
    const res = this.ch.publish(exchangeName, routingKey || '', Buffer.from(JSON.stringify(message)))
    if (res) return true
    else {
