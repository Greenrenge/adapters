...
module.exports = class RedisPubSubInputAdapter {
  ...
  setHandlers() {
    ...
    this.handlers = Object.keys(this._handlers)
      .map(c => ({
        [this.toStream(c)]: { handler: this._handlers[c], channel: c },
      }))
      .reduce(
        (p, c) => ({
          ...p,
  ...
  async setting({
    ...
    this.toStream = channel =>
      `${this.uniqueServiceName}_${STREAM_PREFIX}_${channel}`
    ...
    this._setAck = this.acknowledgement
      ? (fullRedisChannel, id) =>
          this.ackRedis.publish(fullRedisChannel, getAck(id))
      : async () => {}
  ...
  async connect() {
    ...
    const streamHandler = (channel, msg) => {
      ...
      task(data)
        .then(() => {
          this._setAck(
            this.toAck(this.handlers[channel].channel),
            getStreamId(msg),
          ).then(n => {
        ...
        .catch(err => {
          logger.debug('error occurred during call handler', {
            name: err.name,
            message: err.message,
            stack: err.stack,
          })
    ...
    this.redis.subscribe(this.streamChannels, function(err, count) {
      if (err) {
        logger.error('error to subscribe', {
          name: err.name,
          message: err.message,
          stack: err.stack,
        })
