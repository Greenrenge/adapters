...
module.exports = class RabbitMQInputAdapter {
  ...
  /**
     ...
     */
  async setting ({ connectionString, exchangeName, exchangeType = 'topic', queueName, namespace = '', prefetch = 10, acknowledgement = true, maxPriority = 0, ignoreRoutingKey = false, autoReconnect = false, delayReconnectMS = 5000, logger: customLogger }) {
    ...
    if (!queueName || !exchangeName || !connectionString || !queueName.length || !exchangeName.length || !connectionString.length) {
      logger.error('an exchangeName,queueName and connectionString must be specified!')
      throw new Error('an exchangeName,queueName and connectionString must be specified!')
    }
    ...
    try {
      if (this.ignoreRoutingKey) {
        ...
        if (this.exchangeType === 'topic') {
          ...
          await this.mq.bindQueue(this.ch, this.queueName, this.exchangeName, `${this.namespace ? this.namespace + '.' : ''}#`)
          logger.debug('bind queue complete')
          // need time to really works if first time binding
          await delay(2000)
        } else {
          await this.mq.bindQueue(this.ch, this.queueName, this.exchangeName, '')
          logger.debug('bind queue complete')
          // need time to really works if first time binding
          await delay(2000)
        }
  ...
  async _receiveData () {
    try {
      const readMessage = this.acknowledgement ? this.mq.readMessageWithAck.bind(this.mq) : this.mq.readMessageWithNoAck.bind(this.mq)
      if (this.ignoreRoutingKey) {
