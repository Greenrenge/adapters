...
module.exports = class RedisPubSubOutputAdapter {
  ...
  _initAck(channel) {
    ...
    this.ackRedis.subscribe(fullChannel, function(err, count) {
      if (err) {
        logger.error('error to subscribe in _initAck', {
          name: err.name,
          message: err.message,
          stack: err.stack,
        })
  ...
  publish({ channel, data }) {
    if (this.acknowledgement) {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.redis
          .publish(this.toStream(channel), JSON.stringify(sentData))
          .then(() => {
            // console.log('published')
            setTimeout(
              reject,
              this.maxWaitAckMillisec,
              new Error('timeout for acknowledgement'),
            )
          })
      })
    }
    return this.redis.publish(
      this.toStream(channel),
      JSON.stringify(setStreamData(data)),
    )
  }
